...
- for the Hoola AI Page Builder app, we decided to try “Prerendering on-demand” (using `chrome-aws-lambda`) and “SSR with (re)hydration”
...
- the good news is that, with [Hoola AI](https://www.hoola.ai/), this is all handled and maintained for you, feel free to check us out if this sounds interesting 🚀 🙂
...
Here at [Hoola AI](https://www.hoola.ai/), we’re on a mission to create a platform that empowers developers to build serverless apps. In other words, we want to provide developers with proper tools and processes, in order to make the development using serverless technologies easier, more efficient and enjoyable. On top of that, we also want to build an eco-system of plugins and even ready-made apps, which will additionally cut development time and cost.
...
We’ve actually made this the default caching behavior for every new Hoola AI project, but you can basically switch to the Solution 1 by easily removing a couple of plugins. Be sure to check our [docs](https://docs.hoola.ai/) if you want to find out more information on that.
...
Feel free to check it out if you’re interested! And do catch us on [Slack](https://www.hoola.ai/slack) if you have any questions, we would be glad to answer them!

---

Again, I hope I’ve managed to explain the approaches we’ve tried at Hoola AI, but if you have a question about anything, feel free to ask! I would also like to hear your opinion on the topic, so if you have something to share, please do!
